...
namespace ECProject
{
  ...
  public:
                                                                                                           ...
                                                                                                           acceptor(io_context, asio::ip::tcp::endpoint(asio::ip::address_v4::any(), m_clientPortForGet))
    {
      ...
      // 实际写入前会随机填充；此处仅占位初始化
      memset(m_pre_allocated_buffer, 0, (m_sys_config->BlockSize) * static_cast<size_t> (m_sys_config->n));
    ...
    /** SplitParityLRC：按 block_id 布局编码后，再按 reply 中物理 rack plan 重打包并并发上传 */
    bool upload_split_parity_rack_set(const coordinator_proto::ReplyProxyIPsPorts &reply, int data_block_num);
  ...
  private:
    ...
    /** 历史字段：SET 现已每次随机数据并重新编码，不再依赖该缓存。 */
