...
namespace ECProject
{
  namespace
  {
    ...
    static grpc::ChannelArguments parix_channel_args()
    {
      ...
      // Keepalive without unlimited pings (HTTP2_MAX_PINGS_WITHOUT_DATA=0 triggers GOAWAY too_many_pings under multi-client load).
                                         ...
                                         int connect_retries = 5)
    {
      ...
      for (int attempt = 0; attempt < connect_retries; ++attempt)
      {
        try
        {
          ...
          if (!ec)
          {
            asio::write(s, asio::buffer(payload, payload_len), ec);
          }
          asio::error_code ign;
