...
namespace ECProject
{
  ...
  */
  ...
  std::shared_ptr<char[]> Client::get(std::string key, size_t &data_size)
  {
    ...
    // 与文档一致：先 getStripe，再按机架数开 accept
    ...
    // 每个机架一条连接批量回传：连接数 = 参与回传的机架数
    const int conn_num = reply.proxyips_size();
    if (conn_num <= 0)
    {
      std::cout << "[Client] get stripe: no rack connection expected" << std::endl;
      return nullptr;
    }

    std::atomic<int> bad_blocks{0};
    std::vector<std::thread> threads;
    threads.reserve(static_cast<size_t>(conn_num));
    for (int c = 0; c < conn_num; c++)
    {
      threads.emplace_back([this, data_ptr_array, block_size, data_size, &bad_blocks]() mutable {
        ...
        asio::read(socket_data, asio::buffer(&num_blocks, sizeof(uint32_t)), error);
        if (error)
        {
          std::cout << "[Client] get stripe read num_blocks failed: " << error.message() << std::endl;
        ...
        for (uint32_t b = 0; b < num_blocks; b++)
        {
          ...
          if (error)
          {
            std::cout << "[Client] get stripe read block_id failed: " << error.message() << std::endl;
            bad_blocks.fetch_add(1);
            break;
          }
