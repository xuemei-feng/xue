...
namespace ECProject
{
  ...
  */
  ...
  bool Client::set()
  {
    ...
    // SplitParityLRC：逻辑块序编码后，按物理机架重打包，并发发到各接收机架 proxy
    if (m_sys_config->CodeType == "SplitParityLRC")
    {
      if (reply.slice_block_counts_size() != slice_count)
      {
        std::cout << "[SET] SplitParityLRC: slice_block_counts size mismatch" << std::endl;
        return false;
      }
      ...
      for (int i = 0; i < slice_count; ++i)
      {
        ...
        {
          std::cout << "[SET] SplitParityLRC: slice " << i << " size mismatch expect=" << expect
                    << " blocks=" << blk_cnt << std::endl;
          return false;
        }
        if (id_cursor + blk_cnt > reply.slice_block_ids_size())
        {
          std::cout << "[SET] SplitParityLRC: slice_block_ids underflow" << std::endl;
          return false;
        }
        ...
        for (int j = 0; j < blk_cnt; ++j)
        {
          ...
          if (bid < 0 || bid >= n)
          {
            std::cout << "[SET] SplitParityLRC: invalid block_id=" << bid << std::endl;
            return false;
          }
