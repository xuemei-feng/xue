...
namespace ECProject  //定义一个名为 ECProject 的命名空间，防止命名冲突
{
  ...
  std::vector<proxy_proto::AppendStripeDataPlacement> CoordinatorImpl::generate_add_plans(Stripe *stripe)
  {
    ...
    // SplitParityLRC SET：按物理机架 (map2cluster) 派单，proxy 只写本架 DN
    if (m_sys_config->CodeType == "SplitParityLRC")
    {
      ...
      for (auto &kv : cluster_to_blocks)
      {
        ...
        plan.set_stripe_id(stripe->stripe_id);
        plan.set_append_size(static_cast<uint64_t>(block_ids.size()) *
        ...
        for (int block_id : block_ids)
        {
          ...
          assert(blk->map2cluster == cluster_id &&
                 "SplitParityLRC SET plan must only contain blocks on the target rack");
      ...
      coordinator_proto::ReplyProxyIPsPorts *proxyIPPort)
  {
    ...
    for (const auto &plan : add_plans)
    {
      ...
      {
        auto *slice_blocks = proxyIPPort->add_slice_block_lists();
        for (int bi = 0; bi < plan.blockids_size(); ++bi)
        {
          slice_blocks->add_block_ids(plan.blockids(bi));
        }
      }
      sum_append_size += plan.append_size();
    }
