...
namespace ECProject  //定义一个名为 ECProject 的命名空间，防止命名冲突
{
  ...
  void CoordinatorImpl::initialize_cord_d_stripe_placement(Stripe *stripe)
  {
    // CordD placement:
    // - global parity blocks -> cluster (stripe_id % ClusterNum)
    // - local parity blocks -> cluster ((stripe_id % ClusterNum) + 1) % ClusterNum
    // - data blocks -> randomly among the remaining clusters, data count per cluster <= r + 1
    Block *blocks_info = new Block[stripe->n];
    assert(stripe->object_keys.size() == 1);
    ...
    if (cluster_num <= 0)
    {
      throw std::runtime_error("ClusterNum must be positive for CordD placement");
    }

    const int global_cluster = stripe->stripe_id % cluster_num;
    const int local_cluster = (stripe->stripe_id % cluster_num + 1) % cluster_num;

    std::vector<int> data_clusters;
    data_clusters.reserve(cluster_num > 2 ? cluster_num - 2 : 0);
    for (int c = 0; c < cluster_num; ++c)
    {
      if (c != global_cluster && c != local_cluster)
        data_clusters.push_back(c);
    }
    if (data_clusters.empty())
    {
      throw std::runtime_error("CordD placement requires at least 3 clusters");
    }

    const int global_parity_group_id = stripe->z;
    const int max_data_per_cluster = stripe->r + 1;
    ...
    std::vector<int> data_block_ids(stripe->k);
    std::iota(data_block_ids.begin(), data_block_ids.end(), 0);
    const int max_attempts = 256;
    bool placed = false;
    for (int attempt = 0; attempt < max_attempts && !placed; ++attempt)
    {
      std::shuffle(data_block_ids.begin(), data_block_ids.end(), gen);
      std::map<int, int> cluster_data_count;
      bool ok = true;
      for (int bid : data_block_ids)
      {
        ...
        for (int cid : candidate_clusters)
        {
          if (cluster_data_count[cid] + 1 <= max_data_per_cluster)
          {
            assigned_cluster[bid] = cid;
            cluster_data_count[cid]++;
            assigned = true;
            break;
          }
        }
    ...
    if (!placed)
    {
      throw std::runtime_error("CordD placement failed to satisfy data block per-cluster constraint");
    }
      ...
      coordinator_proto::ReplyProxyIPsPorts *proxyIPPort)
  {
    ...
    if (curStripeOffset.offset == 0)
    {
      ...
      else if (code_type == "CordD")
      {
        initialize_cord_d_stripe_placement(&t_stripe);
      }
      else
      {
      ...
      coordinator_proto::ReplyProxyIPsPorts *proxyIPPort)
  {
    ...
    assert((code_type == "UniLRC" || code_type == "AzureLRC" || code_type == "OptimalLRC" || code_type == "UniformLRC" || code_type == "RandomLRC" || code_type == "CordD") && "Error: code type must be UniLRC, AzureLRC, OptimalLRC, UniformLRC, RandomLRC, or CordD!");
