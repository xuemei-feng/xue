...
namespace ECProject
{
  ...
  // tag-based data acceptor + worker pool
  ...
  // ──────── tag-based data acceptor: single accept loop + worker pool ────────
    ...
    try
    {
      ...
      for (int j = 0; j < slice_num; ++j)
      {
        ...
        if (!CordRangeWriteToDatanode(placement->blockkeys(j), placement->blockids(j),
                                      static_cast<int>(placement->offsets(j)),
                                      slices[static_cast<size_t>(j)],
                                      slen, placement->datanodeip(j).c_str(),
                                      placement->datanodeport(j)))
        {
      ...
      if (!CordDeltaBlobToDatanode(placement->delta_blob_key(), delta_concat.data(),
                                   delta_concat.size(),
  ...
  {
    std::cout << "[CoRD-DATA][" << proxy_ip_port << "] tag acceptor started" << std::endl;
    while (true)
    {
      try
      {
        ...
        // read tag: [4 bytes key_len (network order)] [key_bytes]
        uint32_t key_len_net = 0;
        asio::error_code ec;
        asio::read(sock, asio::buffer(&key_len_net, sizeof(key_len_net)), ec);
        if (ec) { sock.close(); continue; }
        uint32_t key_len = ntohl(key_len_net);
        if (key_len == 0 || key_len > 1024) { sock.close(); continue; }
        ...
        if (!placement) {
          std::cout << "[CoRD-DATA] unknown tag=" << tag << std::endl;
          sock.close();
          continue;
        }

        // wait for worker slot, then dispatch
        {
      ...
      {
        std::cout << "[CoRD-DATA] acceptor error: " << e.what() << std::endl;
      }
    }
  }
      ...
      proxy_proto::SetReply *response)
  {
    ...
    {
      std::lock_guard<std::mutex> lk(g_cord_data_placement_mu);
      g_cord_data_placements[placement_copy->key()] = placement_copy;
    }
