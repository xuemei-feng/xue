...
bool
ECProject::get_local_decode_plan(int k, int r, int z, std::string code_type, int failed_block_id,
                                 const std::vector<int> &source_block_ids, std::vector<int> &out_factors)
{
    out_factors.clear();
    int nrows = k + r + z;
    if (failed_block_id < 0 || failed_block_id >= nrows || source_block_ids.empty())
        return false;

    std::vector<unsigned char> gen_matrix(static_cast<size_t>(k) * nrows, 0);
    if (code_type == "UniLRC") {
        gen_unilrc_matrix(gen_matrix.data(), k, r, z);
    } else if (code_type == "AzureLRC" || code_type == "RandomLRC" || code_type == "SplitParityLRC" || code_type == "CordXueLRC") {
        gen_azure_lrc_matrix(gen_matrix.data(), k, r, z);
    } else if (code_type == "OptimalLRC") {
        gen_optimal_lrc_matrix(gen_matrix.data(), k, r, z);
    } else if (code_type == "UniformLRC") {
        gen_uniform_lrc_matrix(gen_matrix.data(), k, r, z);
    } else {
        return false;
    }

    const int ncand = static_cast<int>(source_block_ids.size());

    // Columns of interest: data columns spanned by the failed row or any source row.
    std::vector<int> C;
    for (int c = 0; c < k; c++) {
        bool nz = gen_matrix[static_cast<size_t>(failed_block_id) * k + c] != 0;
        if (!nz) {
            for (int s : source_block_ids) {
                if (gen_matrix[static_cast<size_t>(s) * k + c] != 0) { nz = true; break; }
            }
        }
        if (nz) C.push_back(c);
    }
    ...
    // Augmented system (dim x (ncand+1)): solve sum_s x_s * row_s|_C = row_failed|_C.
    const int W = ncand + 1;
    std::vector<unsigned char> aug(static_cast<size_t>(dim) * W, 0);
    for (int rr = 0; rr < dim; rr++) {
        for (int s = 0; s < ncand; s++)
            aug[static_cast<size_t>(rr) * W + s] = gen_matrix[static_cast<size_t>(source_block_ids[s]) * k + C[rr]];
        aug[static_cast<size_t>(rr) * W + ncand] = gen_matrix[static_cast<size_t>(failed_block_id) * k + C[rr]];
    }

    // Gaussian elimination over GF(256).
    std::vector<int> pivot_col(dim, -1);
    int rpiv = 0;
    for (int col = 0; col < ncand && rpiv < dim; col++) {
        int sel = -1;
        for (int i = rpiv; i < dim; i++) {
            if (aug[static_cast<size_t>(i) * W + col] != 0) { sel = i; break; }
        }
        if (sel == -1)
            continue;
        if (sel != rpiv) {
            for (int c = 0; c < W; c++)
                std::swap(aug[static_cast<size_t>(rpiv) * W + c], aug[static_cast<size_t>(sel) * W + c]);
        }
        unsigned char inv = gf_inv(aug[static_cast<size_t>(rpiv) * W + col]);
        for (int c = col; c < W; c++)
            aug[static_cast<size_t>(rpiv) * W + c] = gf_mul(aug[static_cast<size_t>(rpiv) * W + c], inv);
        for (int i = 0; i < dim; i++) {
            if (i == rpiv)
                continue;
            unsigned char f = aug[static_cast<size_t>(i) * W + col];
            if (f == 0)
                continue;
            for (int c = col; c < W; c++)
                aug[static_cast<size_t>(i) * W + c] ^= gf_mul(f, aug[static_cast<size_t>(rpiv) * W + c]);
        }
        pivot_col[rpiv] = col;
        rpiv++;
    }

    // Consistency: any all-zero source row must have a zero target, else unsolvable.
    for (int i = rpiv; i < dim; i++) {
        if (aug[static_cast<size_t>(i) * W + ncand] != 0)
            return false;
    }

    out_factors.assign(ncand, 0);
    for (int i = 0; i < rpiv; i++)
        out_factors[pivot_col[i]] = static_cast<int>(aug[static_cast<size_t>(i) * W + ncand]);
    return true;
}