...
REMOTE_COMMAND="cd ${REPO_ROOT} && BW_MATRIX_VERBOSE=${V} /bin/bash limit_bw_matrix.sh"
...
# 172.16.2.31 / 172.16.2.32 不做限速：勿加入 proxy_hosts；limit_bw_matrix.sh 也会自动跳过。
# proxy_hosts 应为 6 个 cluster proxy，与 CLUSTER_IPS 一致。
# 假定各节点已通过 update_all.sh（rsync）与本机 REPO_ROOT 对齐，无需 pdcp 分发。
